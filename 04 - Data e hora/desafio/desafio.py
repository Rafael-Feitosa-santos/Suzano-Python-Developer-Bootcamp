...
class Historico:
    ...
    def adicionar_transacao(self, transacao):
        self._transacoes.append({
            ...
            "data": datetime.now(sao_paulo_tz).strftime("%d-%m-%Y %H:%M:%S"),
        })
    ...
    def transacoes_do_dia(self):
        data_atual = datetime.now(sao_paulo_tz).date()
        return [
...
def log_transacao(func):
    def envelope(*args, **kwargs):
        ...
        print(f"{datetime.now(sao_paulo_tz)}: {func.__name__.upper()}")
        return resultado
...
def recuperar_conta_cliente(cliente):
    if not cliente.contas:
        ...
        return
    return cliente.contas[0]  # Pega a primeira conta
